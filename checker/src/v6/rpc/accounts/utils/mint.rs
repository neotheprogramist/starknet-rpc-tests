...
use crate::v6::rpc::accounts::creation::structs::{MintRequest, MintRequest2, MintResponse};
...
pub async fn mint_2(base_url: Url, mint_request: &MintRequest2) -> Result<MintResponse, MintError> {
    let mint_url = match base_url.join("mint") {
        Ok(url) => url,
        Err(e) => return Err(MintError::JoinUrlError(e)),
    };
